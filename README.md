...
**Agent 工具轮次（可选）**：侧栏 `POST /api/agent/chat` 中模型与工具的多轮循环默认最多 **5** 轮。环境变量 **`CF_AGENT_MAX_TOOL_ROUNDS`**：正整数 **1～1_000_000**；设为 **`0`、`-1` 或 `unlimited`/`inf`** 表示**不限制轮次**（内部仍有工程安全顶，一般会在终答前结束）。深度读代码时可设大或设 0；无上限会显著增加延迟与 API 费用。
